...
def compute_lower_rank_approx_via_SVD(data_matrix, desired_quality) :
    ...
    frobenius_norm = sum(map(lambda i : i * i, s))
    k = len(s)

    #sum of squared singular values up to i divided by frobenius norm  is basically the quality of the matrix
    for i in range(len(s)):
      if(sum(map(lambda i : i * i, s[:i+1]))/frobenius_norm > desired_quality):
        k = i
        break
    ...
    smat = np.diag(s_k)
    current_approximant = np.dot(u_k, np.dot(smat, vh_k))
...
# etc etc etc
#
def division_of_matrix(div, n, k):
  rows = []
  sum_of_rows = 0
  #if div = 10/4 = 2.5, then we will have 4 sub-matrixes, two of which will have 2 rows and other two sub-matrix will have 2 rows as well
  if (div == (int(div)+0.5)):
      for i in range(int(k/2)):
        rows.append(int(div)+1)
        rows.append(int(div))

  #if div = 12/4 = 3, then, we will just have 4 sub-matrixes with 3 rows
  elif(div == int(div)):
    for i in range(k):
      rows.append(int(div))

  # if div = 11/4 = 2.84, then we know that there will be much submatrixes with 3 rows than with 2
  elif (div > (int(div)+0.5)):
    #appending 3 until we hit the last block matrix, which will have 2 rows
    while (sum_of_rows < (n - int(div))):
      rows.append(int(div+1))
      sum_of_rows = sum_of_rows + int(div+1)

    rows.append(int(div))
    if(len(rows) != k):
      print("the division is incorrect")

  # if div=9/4=2.25, then we know that there will be more sub-matrixes with 2 rows
  else:
    while (sum_of_rows < (n - int(div+1))):
      rows.append(int(div))
      sum_of_rows = sum_of_rows + int(div)
...
def compute_image_block(data_matrix, k) :
    ...
    rows = division_of_matrix(n/k, n, k)
    columns = division_of_matrix(d/k, d, k)

    image_block = []
    #putting everything in the right format
    for i in range(len(rows)):
      inner_list = []
      for j in range(len(columns)):
        inner_list.append((rows[i], columns[j]))
      image_block.append(inner_list)
...
def get_approximation(data_matrix, k, quality, verbose) :
    ...
    i = 0
    j = 0
    B = np.zeros((data_matrix.shape))
    for row in image_block:
      for shape in row:
        #filling B matrix
        B[i:i+shape[0], j:j+shape[1]] = compute_lower_rank_approx_via_SVD(data_matrix[i:i+shape[0], j:j+shape[1]], quality)
        j = j + shape[1]
      j = 0
      i = i + shape[0]
    ...
    return B #reconstruct_data_from_image_block(blocked_data_matrix, k)
